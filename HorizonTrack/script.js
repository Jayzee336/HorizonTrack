...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function renderLiveAttendance(attendanceData, currentEmployee) {
    ...
    // Create or get the entries container
    let entriesContainer = liveAttendanceDiv.querySelector('.attendance-entries-container');
    if (!entriesContainer) {
      entriesContainer = document.createElement('div');
      entriesContainer.className = 'attendance-entries-container';
      liveAttendanceDiv.appendChild(entriesContainer);
    }
    ...
    // Filter attendanceData to only today's entries for all employees
    const todaysEntries = attendanceData.filter(entry => entry.date === today);

    // Clear existing entries
    entriesContainer.innerHTML = '';

    // Separate IN and OUT entries
    const inEntries = todaysEntries.filter(entry => entry.status === 'IN');
    const outEntries = todaysEntries.filter(entry => entry.status === 'OUT');
    ...
    // Create IN section if there are IN entries
    if (inEntries.length > 0) {
      const inSection = document.createElement('div');
      inSection.className = 'attendance-section';
      inSection.innerHTML = '<h3 class="section-heading time-in-heading">Time In</h3>';
      const inContainer = document.createElement('div');
      inContainer.className = 'section-entries';
      inEntries.forEach(entry => {
        const entryDiv = document.createElement('div');
        entryDiv.className = 'attendance-entry jaycein';
        entryDiv.innerHTML = `
          <div class="attendance-sbon-light-in">${entry.status}</div>
          ...
          <div class="jaycein-child"></div>
        `;
        inContainer.appendChild(entryDiv);
      });
      inSection.appendChild(inContainer);
      entriesContainer.appendChild(inSection);
    }

    // Create OUT section if there are OUT entries
    if (outEntries.length > 0) {
      const outSection = document.createElement('div');
      outSection.className = 'attendance-section';
      outSection.innerHTML = '<h3 class="section-heading time-out-heading">Time <span class="out-text">Out</span></h3>';
      const outContainer = document.createElement('div');
      outContainer.className = 'section-entries';
      outEntries.forEach(entry => {
        const entryDiv = document.createElement('div');
        entryDiv.className = 'attendance-entry jayceout';
        entryDiv.innerHTML = `
          <div class="attendance-sbon-light-out">${entry.status}</div>
          <div class="jayce-bruce-nanual-container">
            <span class="span">${entry.time} ${entry.date}</span>
            <span> - ${entry.fullName} - ${entry.employeeID}</span>
          </div>
          <div class="jayceout-child"></div>
        `;
        outContainer.appendChild(entryDiv);
      });
      outSection.appendChild(outContainer);
      entriesContainer.appendChild(outSection);
    }
  }
});
